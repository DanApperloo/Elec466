...
#if defined(COMPARE_VALUES)
static unsigned char EXPECTED1[8] = {
    0x09, 0x2a, 0xf1, 0x41, 0xe2, 0x93, 0x61, 0xd5
};

static unsigned char EXPECTED2[64] = {
    0x64, 0x30, 0x94, 0xc5, 0xda, 0xd2, 0xf6, 0xda, 0x49, 0x6d, 0x67,
    0xf1, 0x16, 0x55, 0xb3, 0xea, 0xee, 0xa2, 0xc0, 0x30, 0x2b, 0xb5,
    0x4f, 0x05, 0x9e, 0xa4, 0x58, 0xac, 0x97, 0x3b, 0xb9, 0xa0, 0x25,
    0xb7, 0x56, 0xfe, 0x82, 0x73, 0xbb, 0x22, 0xd4, 0x31, 0x36, 0x60,
    0x7f, 0x41, 0xe9, 0x47, 0x97, 0xb9, 0x5e, 0x27, 0x99, 0x3e, 0x73,
    0xf0, 0x28, 0xda, 0xb5, 0x25, 0xda, 0xe4, 0x61, 0x84
};

static unsigned char* EXPECTED_ARRAY[2] = {
    EXPECTED1,
    EXPECTED2
};
#endif
...
)
{
...
#endif // TEST_OUTPUT
#ifdef LOG_OUTPUT
    LOG("\r\nHW_MULT:     Input:  b = %u     c = %u\r\n", b, c);
#endif // LOG_OUTPUT

    //LOG("\r\nSW_MULT:    Starting Multiplication.\r\n");

    out_data_1.write(b);
    out_data_2.write(c);  
    hw_mult_enable.write(true);

    // Wait till hw_mult_done is asserted
    wait(hw_mult_done.posedge_event());
    ...
    // Wait till hw_mult_done is deasserted
    wait(hw_mult_done.negedge_event());

    //LOG("SW_MULT:    Finished.\r\n");
#ifdef COMPARE_OUTPUT
    ...
    test_a0 = (NN_DIGIT)bLow * (NN_DIGIT)cLow;
    t = (NN_DIGIT)bLow * (NN_DIGIT)cHigh;
    u = (NN_DIGIT)bHigh * (NN_DIGIT)cLow;
    test_a1 = (NN_DIGIT)bHigh * (NN_DIGIT)cHigh;
    
    if ((t += u) < u) test_a1 += TO_HIGH_HALF (1);
    u = TO_HIGH_HALF (t);
    
    if ((test_a0 += u) < u) test_a1++;
    test_a1 += HIGH_HALF (t);

    if ( (test_a0 != a[0]) || (test_a1 != a[1]) )
    {
        LOG("HW_MULT:    Input:  b = %u     c = %u\r\n", b, c);
        LOG("HW_MULT:    Output was not correct. Desired: %u %u   Calculated: %u %u\r\n",
            test_a1, test_a0, a[1], a[0]);
    }
#endif
#ifdef LOG_OUTPUT
    LOG("HW_MULT:    Output: a0 = %u    a1 = %u\r\n", a[0], a[1]);
#endif // LOG_OUTPUT
#ifdef TEST_OUTPUT
    if ( attempt++ > 200 )
    {
    ...
    while (keys_exchanged != NUM_KEYEXCHANGE) 
      {
        ...
        /*** Generate parameters ***/
        //LOG("\r\nDH_SW: Key %d: Generating DH Params.\r\n", keys_exchanged);
        if ((status = R_GenerateDHParams
        ...
        /*** Setup agreement ***/
        //LOG("\r\nDH_SW: Key %d: Setting up DH Agreement.\r\n", keys_exchanged);
        {
          params = &PARAMS1;
    ...
    
        {
          ...
          /* Set up a break point with a do {} while (0) so that we can
              zeroize the sensitive buffers before exiting.
           */
          //LOG("\r\nDH_SW: Key %d: Computing DH Agreed Key.\r\n", keys_exchanged);
          do {
...
#if defined(COMPARE_VALUES)
          CompareBigInteger(agreedKey, EXPECTED_ARRAY[keys_exchanged-1], params->primeLen);
#endif
