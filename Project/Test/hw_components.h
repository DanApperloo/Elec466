...
SC_MODULE(shifter16)
{
    ...
    void process_shift(void)
    {
        ...
        if ( true == enable.read() )
        {
            ...
            {
                // Shift Left
                temp = (in_sample << NN_HALF_DIGIT_BITS) & 0xffff0000;
                out.write(temp);
            ...
            {
                // Shift Right
                out.write(in_sample.range(31, 16));
            }
            else
            {
                out.write(in_sample);
...
SC_MODULE (multiplier)
{
    ...
    void process_mult(void)
    {
        ...
        sc_uint<NN_DIGIT_BITS> calculation = a_sample.to_uint() * b_sample.to_uint();
...
SC_MODULE (adder)
{
    ...
    void process_add(void)
    {
        ...
        if ( true == enable.read() )
        {
            printf("Adding a: %u    b: %u\r\n", a_sample.to_int(), b_sample.to_int());
...
SC_MODULE (register_file)
{
    ...
    SC_CTOR (register_file)
    {
        ...
        sensitive << reg_array[0] << reg_array[1] << reg_array[2]
                  << reg_array[3] << reg_array[4] << reg_array[5]
                  << read_enable << read_a_select << read_b_select;
